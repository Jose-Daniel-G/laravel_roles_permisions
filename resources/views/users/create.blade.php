                    ...
                    <form action="{{ route('users.store') }}" method="POST">
                        ...
                        <div class="my-3">
                            <input value="{{ old('name') }}" type="text" placeholder="name" name="name"
                                id="name" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                            @error('name')
                        ...
                        <div class="my-3">
                            <input value="{{ old('email') }}" type="text" placeholder="email" name="email"
                                id="email" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                            @error('email')
                                <p class="text-red-400 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                        @if ($roles->isNotEmpty())
                            @foreach ($roles as $role)
                                <div class="mt-3">
                            {{-- <input {{ $hasPermissions->contains($role->name) ? 'checked':''}} type="checkbox" name="role[]" id="role-{{$role->id}}"  class="rounded" value="{{ $role->name }}"> --}}
